  ...
  while (characters.length) {
    ...
    if (end < characters.length) {
      for (let i = end; i > end * 0.7; i--) {
        if (/[。！？\n]/.test(characters[i - 1])) { end = i; break }
      }
    }
...
export function decodeText(bytes: Uint8Array): string {
  ...
  catch { return new TextDecoder('shift-jis', { fatal: true }).decode(bytes) }
}
...
const elements = (doc: Document | Element, name: string) => Array.from(doc.getElementsByTagNameNS('*', name))
...
function archivePath(base: string, href: string): string {
  ...
  if (!path || /^(?:[a-z]+:|\/|\\)/i.test(path) || path.includes('\\')) throw new Error('This EPUB contains an unsafe content path.')
  const parts = base.split('/').slice(0, -1)
  for (const part of path.split('/')) {
    if (part === '..') { if (!parts.length) throw new Error('This EPUB contains an unsafe content path.'); parts.pop() }
...
async function readArchive(bytes: Uint8Array) {
  ...
  if (end < 0 || end < bytes.length - 65557) throw new Error('This EPUB is not a valid ZIP archive.')
  const count = view.getUint16(end + 10, true)
  let offset = view.getUint32(end + 16, true)
  if (count >= 5000 || view.getUint16(end + 4, true) || view.getUint16(end + 6, true)) throw new Error('This EPUB has too many files or uses an unsupported archive format.')
  let total = 0
  const checksums = new Map<string, number>()
  for (let i = 0; i < count; i++) {
    if (offset + 46 > end || view.getUint32(offset, true) !== 0x02014b50) throw new Error('This EPUB archive is damaged.')
    ...
    if (/^(?:\/|\\|[a-z]:)/i.test(name) || name.includes('\\') || name.split('/').includes('..') || checksums.has(name)) throw new Error('This EPUB contains unsafe or duplicate file paths.')
    total += size
    if (size > 16 * MB || total > 200 * MB) throw new Error('This EPUB expands beyond the supported size limits (16 MB per file, 200 MB total).')
  ...
  unzipSync(bytes, { filter: file => {
    if (!checksums.has(file.name) || file.originalSize > 16 * MB) throw new Error('This EPUB archive is damaged.')
    return false
  } })
  const files = await new Promise<Record<string, Uint8Array>>((resolve, reject) => unzip(bytes, (error, data) => error ? reject(error) : resolve(data)))
  for (const [name, data] of Object.entries(files)) {
    if (data.length > 16 * MB || crc32(data) !== checksums.get(name)) throw new Error('This EPUB failed its integrity check. Try downloading it again.')
  }
  return files
}
...
function extractChapter(text: string) {
  ...
  const doc = xml(text)
  for (const element of doc.querySelectorAll('rt, rp, script, style, head, svg, nav')) element.remove()
  const heading = doc.querySelector('h1, h2, h3')?.textContent?.trim()
  const body = elements(doc, 'body')[0] ?? doc.documentElement
  for (const element of body.querySelectorAll('p, div, section, h1, h2, h3, li, br')) element.append('\n\n')
  return { text: clean(body.textContent ?? '').replace(/[ \t]+/g, ' ').replace(/\n\s*\n(?:\s*\n)+/g, '\n\n'), heading }
}
...
async function importEpub(bytes: Uint8Array) {
  ...
  for (const ref of elements(pkg, 'itemref')) {
    ...
    if (encrypted.has(path)) throw new Error('This EPUB has encrypted chapters. Import a DRM-free edition.')
    const chapter = extractChapter(get(path))
    pages.push(...paginate(chapter.text, chapter.heading || `Chapter ${pages.length + 1}`))
  }
  const coverId = elements(pkg, 'meta').find(meta => meta.getAttribute('name') === 'cover')?.getAttribute('content')
  const coverItem = [...manifest.values()].find(item => item.getAttribute('properties')?.split(' ').includes('cover-image')) ?? manifest.get(coverId ?? '')
...
async function importPdf(bytes: Uint8Array) {
  ...
  try {
    ...
    if (document.numPages > 5000) throw new Error('PDFs with more than 5,000 pages are not supported.')
    const pages: Page[] = []
    ...
    if (!pages.some(page => page.text)) throw new Error('This PDF has no selectable text. Scanned pages need OCR before importing.')
    return { pages }
...
export async function importBook(file: File): Promise<Book> {
  ...
  if (format !== 'txt' && format !== 'epub' && format !== 'pdf') throw new Error('Choose a PDF, EPUB, or TXT file.')
  const bytes = new Uint8Array(await file.arrayBuffer())
  const base: Book = { id: crypto.randomUUID(), title: file.name.replace(/\.[^.]+$/, ''), author: 'Your collection', format, pages: [], position: 0, readPages: [], completed: false, addedAt: new Date().toISOString(), tone: Math.floor(Math.random() * 4), highlights: [] }
  const content: { pages: Page[]; title?: string; author?: string; cover?: string } = format === 'epub' ? await importEpub(bytes) : format === 'pdf' ? await importPdf(bytes) : { pages: paginate(decodeText(bytes)) }
  const book = { ...base, ...content, title: content.title || base.title, author: content.author || base.author }
