...
async function readData<T>(name: string): Promise<T> {
  ...
  if (!response.ok)
    throw new Error(
      'The local dictionary could not be loaded. Try again after the page finishes loading.',
    )
  const bytes = new Uint8Array(await response.arrayBuffer())
  // Some static servers send .gz files with Content-Encoding: gzip; fetch has
  // already decompressed those responses. Other hosts serve the raw archive.
  const json = bytes[0] === 0x1f && bytes[1] === 0x8b ? gunzipSync(bytes) : bytes
  return JSON.parse(strFromU8(json))
}

function indexRows(rows: Row[]): Dictionary {
  const entries = rows.map(([id, word, reading, meanings, pos, common]) => ({
    id,
    word,
    reading,
    meanings,
  ...
  for (const entry of entries)
    for (const form of new Set([entry.word, entry.reading]))
      index.set(form, [...(index.get(form) ?? []), entry])
  return { entries, index }
}

export function commonDictionary() {
  return (commonPromise ??= readData<Row[]>('common')
    .then(indexRows)
    .catch((error) => {
...
export function dictionaryForms(text: string): string[] {
  ...
  const polite: Record<string, string> = {
    い: 'う',
    き: 'く',
    ぎ: 'ぐ',
    し: 'す',
  ...
  for (const [suffix, endings] of rules) {
    ...
    const ending = (
      ['ました', 'ます', 'ません'].includes(suffix)
        ? polite
        : ['ない', 'なかった'].includes(suffix)
          ? negative
          : {}
    )[stem.slice(-1)]
  ...
  for (const [surface, lemma] of [
    ...
    if (text === surface) result.add(lemma)
  return [...result]
}
...
export async function lookup(text: string): Promise<Entry[]> {
  ...
  const direct = [
    ...(common.index.get(query) ?? []),
    ...((await rareDictionary(query)).index.get(query) ?? []),
  ]
...
export async function kanjiFor(text: string): Promise<Kanji[]> {
  kanjiPromise ??= readData<[string, string[], string[], string[]][]>('kanji')
    .then(
      (rows) =>
        new Map(
          rows.map(([character, onyomi, kunyomi, meanings]) => [
            character,
    ...
    .catch((error) => {
      kanjiPromise = undefined
      throw error
    })
  const map = await kanjiPromise
  return [...new Set(text)]
    .map((character) => map.get(character))
    .filter((item): item is Kanji => !!item)
}

export function sentenceFor(word: string, text: string) {
  return (text.split(/(?<=[。！？])|\n/).find((sentence) => sentence.includes(word)) ?? text)
    .trim()
    .slice(0, 220)
}

export async function extractCandidates(
  pages: { page: Page; index: number }[],
): Promise<Candidate[]> {
    ...
    for (let i = 0; i < segments.length; i++) {
      for (let length = Math.min(4, segments.length - i); length >= 1; length--) {
        ...
        if (
          surface.length < 2 ||
          surface.length > 24 ||
          !/^[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}ー]+$/u.test(surface)
        )
          continue
        const entry = dictionaryForms(surface)
          .map((form) => index.get(form)?.[0])
          .find(Boolean)
        if (
          !entry ||
          (!/\p{Script=Han}/u.test(surface) &&
            (entry.word !== surface || surface.length < 3 || /auxiliary|suffix/.test(entry.pos)))
        )
          continue
        if (!seen.has(entry.word)) {
          ...
          candidates.push({
            entry,
            sentence: sentenceFor(surface, page.text),
            page: pageIndex,
            priority: 1,
          })
        }
...
export function speakJapanese(text: string): string | undefined {
  ...
  if (!voices.length)
    return 'No Japanese voice is available. Add a Japanese speech voice in your device settings and reopen the browser.'
