...
export default function Activity({ sessions }: { sessions: Session[] }) {
  ...
  const daily = days.map((day) => ({
    ...day,
    pages: sessions
      .filter((session) => localDay(session.endedAt) === day.key)
  ...
  while (activeDays.has(localDay(day))) {
    streak++
  ...
  return (
    <>
      <section className="page-heading">
        <div className="eyebrow">LITTLE BY LITTLE, EVERY DAY</div>
        <h1>
          Look how far
          <br />
          <em>you’ve come.</em>
        </h1>
        <p>Every page is a step forward.</p>
      </section>
      <div className="stat-grid">
        {[
          [BookOpen, totalPages, 'Pages visited'],
          [Clock3, minutes, 'Minutes reading'],
          [Flame, streak, `Day${streak === 1 ? '' : 's'} in a row`],
      ...
      <section className="activity-chart">
        ...
        <div
          ...
          aria-label={daily
            .map(
              (day) =>
                `${day.date.toLocaleDateString(undefined, { weekday: 'long' })}: ${day.pages} pages`,
            )
            .join(', ')}
        >
          {daily.map((day, i) => (
            <div className={`chart-day ${i === 6 ? 'today' : ''}`} key={day.key}>
              <strong>{day.pages}</strong>
              <div className="bar-area">
                <div
                  className="bar"
                  style={{ height: `${Math.max(day.pages ? 4 : 2, (day.pages / max) * 100)}%` }}
                />
              </div>
      ...
      <section>
        ...
        {!sessions.length ? (
          <Empty icon={<Sprout size={28} />} title="Your story starts with one page">
            Finish a reading session to see your progress here.
          </Empty>
        ) : (
          <div className="session-list">
            {[...sessions].reverse().map((session) => (
              <article className="session-row" key={session.id}>
                ...
                <div>
                  <h3 lang="ja">{session.bookTitle}</h3>
                  <p>
                ...
                <div className="session-metrics">
                  ...
                  <span>
                    {session.seconds < 60 ? '<1 min' : `${Math.floor(session.seconds / 60)} min`}
                    {session.quiz ? ` · Quiz ${session.quiz.correct}/${session.quiz.total}` : ''}
                  </span>
                </div>
              </article>
            ))}
