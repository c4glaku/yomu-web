...
export default function SessionSetup({
  ...
  while (chapterEnd + 1 < book.pages.length && book.pages[chapterEnd + 1].chapter === chapter)
    chapterEnd++
  const end =
    goal === 'free'
      ? null
      : goal === 'chapter'
        ...
        : goal === 'book'
          ? book.pages.length - 1
          : Math.min(book.pages.length - 1, start + Math.max(1, count) - 1)
  return (
    <Modal title="A little time for reading" onClose={onClose}>
      ...
      <div className="field-label">
        ...
        <div className="goal-grid">
          {[
            ['pages', 'A few pages', 'Small steps count'],
            ['chapter', 'This chapter', 'One story at a time'],
            ['book', 'Rest of the book', 'See where it goes'],
            ['free', 'Free reading', 'No finish line'],
      ...
      {goal === 'pages' && (
        <label className="field-label">
          ...
          <input
            type="number"
            min="1"
            max={book.pages.length - start}
            value={count}
            onChange={(event) =>
              setCount(Math.max(1, Math.min(book.pages.length - start, Number(event.target.value))))
            }
          />
        </label>
      )}
      <p className="small muted">
        {end === null
          ? 'Read at your own pace, and finish whenever you like.'
          : `Pages ${start + 1}–${end + 1}. You can pause or finish at any time.`}
      </p>
      <button className="primary full-width" onClick={() => onStart({ start, end })}>
        Let’s read <ArrowRight size={17} />
      </button>
    </Modal>
  )
}
